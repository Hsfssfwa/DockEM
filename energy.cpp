...
Energy_Score::compute_score(DOCKMol & mol,float cutoff)
{
    ...
    for (atom = 0; atom < mol.num_atoms; atom++) {
	   ...
	   //cout<<"potentialB is:"<<potentialB<<endl;
            ...
            //    vdw_scale;
	    vdw_val_tmp =
                ((vdwA[mol.amber_at_id[atom]] * potentialA) -
                (vdwB[mol.amber_at_id[atom]] * potentialB)) *
                vdw_scale;
...
float tor_energy_total (AMBER_TYPER & amber,vector<vector<int>> biolip_matrix,vector<TORSION> &torsions)
{
    vector<int> torsiontypes={1,2,3,32,6,62,63,122};
    float energy_tor_total=0.0;
    int row=0;
    int line = 0;
    for(int i=0;i<torsions.size();i++)
    {
        //cout<<"angle information ))))"<<amber.bond_typer.types[amber.bond_typer.flex_ids[torsions[i].bond_num]].drive_id<<'\t'<<torsions[i].ihedral<<endl;
	//judge the type of torsiontypes
	for (int j=0;j<torsiontypes.size();j++)
	{
	    if(amber.bond_typer.types[amber.bond_typer.flex_ids[torsions[i].bond_num]].drive_id==j)
	    {
		//cout<<"find the row))))))))"<<amber.bond_typer.types[amber.bond_typer.flex_ids[torsions[i].bond_num]].drive_id<<"    "<<j<<endl;
	        row = j;//find the row
		break;
	    }
	    
	}
	line=ceil(torsions[i].ihedral+180.0);
        //cout<<"row and line informaiton "<<row<<"    "<<line<<endl;
	//calcule the energy
	//cout<<"probablity (((("<<(biolip_matrix[row][line]+1)*1.0<<"    "<<(biolip_matrix[row][0]+1)*1.0<<endl;
	energy_tor_total = energy_tor_total + (-(log(((biolip_matrix[row][line]+1)*1.0)/((biolip_matrix[row][0]+1)*1.0))));
	//cout<<biolip_matrix[0][1]*1.0<<"^^^"<<biolip_matrix[0][0]*1.0<<"&&&&"<<-(log((biolip_matrix[0][1]*1.0)/(biolip_matrix[0][0]*1.0)))<<endl;
	//cout<<"now energy_tor_total "<<energy_tor_total<<endl;
    }
    return energy_tor_total;
}
float tor_energy (AMBER_TYPER & amber,vector<vector<int>> biolip_matrix,TORSION torsion)
{
    vector<int> torsiontypes={1,2,3,32,6,62,63,122};
    float energy_tor=0.0;
    int row=0;
    int line = 0;
    //find row
    for (int j=0;j<torsiontypes.size();j++)
    {
	if(amber.bond_typer.types[amber.bond_typer.flex_ids[torsion.bond_num]].drive_id==j)
	{
	    row = j;//find the row
	    break;
	}
    ...
    //find line
    line=ceil(torsion.ihedral+180.0);
    //cout<<"row and line informaiton "<<row<<"    "<<line<<endl;
    
    //cout<<"probablity (((("<<(biolip_matrix[row][line]+1)*1.0<<"    "<<(biolip_matrix[row][0]+1)*1.0<<endl;
    energy_tor = -(log(((biolip_matrix[row][line]+1)*1.0)/((biolip_matrix[row][0]+1)*1.0)));
    return energy_tor;
}
...
/*******************************************************************/
bool hbond_cal(DOCKMol & mol_a,DOCKMol & mol_d,int i,int j,double & hbond_dist, double & hbond_angle)
{
...
float hbond_energy (DOCKMol &mol, DOCKMol &receptor)
{
   ...
   //if(hbond_total >0)
   //{
   // cout<<"hbond energy "<<hbond_total<<endl;
   //}
...
{
    float dis_energy = 0.0;;

    for (int protein_i=0;protein_i<protein_atom_index.size();protein_i++)
    {
        
            for(int ligand_m= 0;ligand_m<mol.num_atoms;ligand_m++)
            {
                if(ave[protein_i][ligand_m] <= 0)
                    continue;

                if(mol.atom_types[ligand_m] == "H")
                    continue;
                int p_index = protein_atom_index[protein_i];
                float distance2= 
                    (receptor.x[p_index] - mol.x[ligand_m])*(receptor.x[p_index] - mol.x[ligand_m])\
                    +(receptor.y[p_index] - mol.y[ligand_m])*(receptor.y[p_index] - mol.y[ligand_m])\
                    +(receptor.z[p_index] - mol.z[ligand_m])*(receptor.z[p_index] - mol.z[ligand_m]);
                float distance_temp = sqrt(distance2);
                //cout<<"distance energy checking:"<< distance_temp<< " "<<(0.5 * (distance_temp - ave[protein_i][ligand_m]) * (distance_temp - ave[protein_i][ligand_m])) / std[protein_i][ligand_m]<<endl;
                dis_energy = dis_energy +  
                (0.5 * (distance_temp - ave[protein_i][ligand_m]) * (distance_temp - ave[protein_i][ligand_m])) / std[protein_i][ligand_m];
            }
...
bool energy (AMBER_TYPER & amber, DOCKMol & mol, DOCKMol & receptor, Energy_Score & c_nrg, string filevdw, string fileflex,string fileflex_drive_tbl,
            ...
            vector<int> protein_atom_index,vector<vector <float> > ave, vector<vector <float> > std, ElectronDensity & DensityMap, vector<poseCoord> &binding_site, vector<float> &centers) {

//bool energy (AMBER_TYPER & amber, DOCKMol & mol, DOCKMol & receptor, Energy_Score & c_nrg, string filevdw, string fileflex,string fileflex_drive_tbl, string grid_file, float cutoff) {
    ...
    id_torsions(mol, vertex, rotatorsions); 
	...
	//cout<<"c_nrg.compute_score finish"<<endl;
        ...
        /***************************************************/
        /***************************************************/
        //close the template energy by zwy 20231224 eve
        ////////////////////////////////////////////////////
        //mol.dis_energy = distance_energy(mol, receptor,protein_atom_index,ave, std);
        //cout<<"mol.dis_energy test "<<mol.dis_energy<<"**************"<<endl;
        /***************************************************/
        float new_clash = 0.0,dis_p = 0.0;
        for (int i = 0; i < binding_site.size(); i++)
        {
            for (int j = 0; j < mol.num_atoms; j++)
            {
                string VR1 = binding_site[i].atom_name;//fin_maty[jj].elt_;
                string VR2 = mol.atom_names[j];// fin_maty[t].elt_;
                float dis = 0.0;
                dis = dis + pow((binding_site[i].x - mol.x[j]), 2);
                dis = dis + pow((binding_site[i].y - mol.y[j]), 2);
                dis = dis + pow((binding_site[i].z - mol.z[j]), 2);
                dis_p = sqrt(dis);
        ...
        distance = sqrt(pow(centers[0] - centers_1[0], 2) + pow(centers[1] - centers_1[1], 2) +
            pow(centers[2] - centers_1[2], 2));

        if (distance <= max) distance = 100 * distance;
        //else if (0.5 < distance <= 2) distance = 500 * distance;
        else distance = 5000 * distance;
        //distance = 50 * distance;

        float inter_clash = 0.0;
        for (int i = 0; i < mol.num_atoms-1; i++)
        {
            for (int j = i + 1; j < mol.num_atoms; j++)
            {
                string VR1 = mol.atom_names[i];//fin_maty[jj].elt_;
                string VR2 = mol.atom_names[j];// fin_maty[t].elt_;
                float dis = 0.0;
                dis = dis + pow((mol.x[i] - mol.x[j]), 2);
                dis = dis + pow((mol.y[i] - mol.y[j]), 2);
                dis = dis + pow((mol.z[i] - mol.z[j]), 2);
                dis_p = sqrt(dis);
                //dis_p = Distance_point(fin_maty[jj].x_, fin_maty[t].x_);
                inter_clash = inter_clash + GetVdwEgCG(VR1, VR2, dis_p);
            }
        }
        //mol.current_EM_score = DensityMap.clCC_2(mol, binding_site);
        mol.current_EM_score = DensityMap.clCC_1(mol);
        float CC_weight = 2000.0;
		...
		//mol.current_score = mol.intral_energy + bindsite_weight * mol.contact_energy + (-10.0) * mol.hb_energy + mol.internal_energy + mol.energy_tor_total + 0.1 * mol.dis_energy;
        //cout<<"I am calculate the energy "<<mol.current_score <<endl;
        //add by wenyizng 2019/12/05
        //mol.current_score = mol.intral_energy + (-10.0) * mol.hb_energy + mol.internal_energy + mol.energy_tor_total;
	    //mol.current_score = mol.intral_energy + (-10.0) * mol.hb_energy + mol.internal_energy;
	    
        //mol.current_score = mol.intral_energy + bindsite_weight * mol.contact_energy + 0.1 * mol.dis_energy;
        //mol.current_score = mol.intral_energy + bindsite_weight * mol.contact_energy + mol.internal_energy;
        //mol.current_score = mol.intral_energy + (-1.0) * mol.hb_energy + mol.internal_energy;
        //mol.current_score=mol.intral_energy;
        //cout<< "mol.intral_energy :" <<mol.intral_energy<<endl;
        //cout<< "mol.internal_energy :" <<mol.internal_energy<<endl;
...
bool energy1(AMBER_TYPER& amber, DOCKMol& mol, DOCKMol& receptor, Energy_Score& c_nrg, string filevdw, string fileflex, string fileflex_drive_tbl,
    string grid_file, float cutoff, vector<vector<int>> biolip_matrix, vector<float>sphx, vector<float>sphy, vector<float> sphz, float bindsite_weight,
    vector<int> protein_atom_index, vector<vector <float> > ave, vector<vector <float> > std, ElectronDensity& DensityMap) {
    ...
    if (c_nrg.compute_score(mol, cutoff)) {
        ...
        //cout<<"mol.dis_energy test "<<mol.dis_energy<<"**************"<<endl;
        /***************************************************/
        mol.current_score = mol.intral_energy + bindsite_weight * mol.contact_energy + (-10.0) * mol.hb_energy + mol.internal_energy + mol.energy_tor_total;// +500 * (1.0 - DensityMap.clCC(mol));
        //mol.current_EM_score = 100 * (1.0 - DensityMap.clCC(mol));
        //cout << DensityMap.clCC(mol) << endl;
        //mol.current_score = mol.intral_energy + bindsite_weight * mol.contact_energy + (-10.0) * mol.hb_energy + mol.internal_energy + mol.energy_tor_total + 0.1 * mol.dis_energy;
        //cout<<"I am calculate the energy "<<mol.current_score <<endl;
        //add by wenyizng 2019/12/05
        //mol.current_score = mol.intral_energy + (-10.0) * mol.hb_energy + mol.internal_energy + mol.energy_tor_total;
