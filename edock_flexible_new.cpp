...
int main(int argc, char ** argv){
    ...
    //theDensityMap.x_min = 23, theDensityMap.y_min = 17, theDensityMap.z_min = 20;
    //theDensityMap.x_max = 28, theDensityMap.y_max = 22, theDensityMap.z_max = 25;   
    //cout << "clCCclCCclCCclCC" << theDensityMap.clCC(mol) << endl;
    cout << "clCC_1clCC_1clCC_1" << theDensityMap.clCC_1(mol) << endl;
    //cout << "clCC_2clCC_2clCC_2" << theDensityMap.clCC_2(mol, receptor) << endl;
   ...
   else if(molvec.size()<initialnum && molvec.size()>0)
   {
    ...
    for (int i = 0; i < tempmachmols[molindex[0]].num_atoms; i++)
    {
        for (int j = i; j < tempmachmols[molindex[0]].num_atoms; j++)
        {
            ...
            if (sqrt(car_x + car_y + car_z) > max)
                max = sqrt(car_x + car_y + car_z);
        }
    }
    cout << "max:  " << max << endl;
    //theDensityMap.cart(centers1, fartbinding, max);
    cout << "centers1:  " << centers1[0] << "  " << centers1[1] << "  " << centers1[2] << endl;
    cout << "molcenters:  " << molcen[0] << "  " << molcen[1] << "  " << molcen[2] << endl;

    string molfile2 = "test1.mol2";
    ofstream clustermol2(molfile2.c_str());
    string str2 = string(argv[1]);
    clustermol2 << "########## Name:" << endl;
    Write_Mol2(tempmachmols[molindex[0]], clustermol2, str2);

    //string molfile3 = "test2.mol2";
    //ofstream clustermol3(molfile3.c_str());
    //string str3 = string(argv[1]);
    //clustermol3 << "########## Name:" << endl;
    //Write_Mol2(mol, clustermol3, str3);
    ...
    //        {
    //            if (x > theDensityMap.x_min && x < theDensityMap.x_max && y > theDensityMap.y_min && y < theDensityMap.y_max && z > theDensityMap.z_min && z < theDensityMap.z_max) continue;
    //            else
    //            {
    //                theDensityMap.density(x, y, z) = 0;
...
//    pointsBx = best_model;
    ...
    float supp_KT0s = 0.005;//;0.0001;1.5
    float supp_KT0e = 0.00025;//;0.00001;0.001
    int supp_num1 = 20;
    int supp_num3 = 500;
    ...
    for (int kkk = 0; kkk < supp_num2; kkk++) // 2
    {
        ...
        for (int jjj = 0; jjj < supp_num1; jjj++)
        {
            ...
            //    cout<<"XXX2"<<endl;  
                ...
                //vector<poseCoord>().swap(fin_matx);
                generatationmol(point_mat, fin_mat, 1, gen);
