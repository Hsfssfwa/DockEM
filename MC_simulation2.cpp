    ...
    while(maxsimulations > 1)
    {
        ...
        if(flexible_flag ==1) 
        {
            ...
            for(int mm = 0;mm <flexible_steps; mm++)
            {
                ...
                //else
                {
                    //if (maxsimulations < 300) flexible_angle = flexible_angle / 3;
                    //if (trans_method) flexible_angle = 90.0;
                    angle=flexible_angle*randgeneration(gen); //randnum from-1 to 1
                ...
                if (energy(amber, endmol,receptor, c_nrg,filevdw.c_str(), fileflex.c_str(),fileflex_drive_tbl.c_str(),grid_file,cutoff,biolip_matrix,sphx,sphy,sphz,bindsite_weight,protein_atom_index,ave,std, DensityMap,binding_sites, centers))
                {
	                ...
	                { 
                        ...
                        if(endmol.current_score<1e+6)
                        {
                            ...
                            cout<< maxsimulations << " flexible movment"<< "  original.current_score: " << original.current_score << "  original.CC: " << original.current_EM_score<<"  original.intral_energy: " << original.intral_energy << "  original.internal_energy: " << original.internal_energy << "  original.contact_energy: " << original.contact_energy <<"  original.energy_tor_total:  " << original.energy_tor_total << "  original.hb_energy: " << original.hb_energy << endl;
                            ...
                            cout << maxsimulations << " flexible movment" << "  endmol.current_score: " << endmol.current_score << "  endmol.CC: " << endmol.current_EM_score << "  endmol.intral_energy: " << endmol.intral_energy << "  endmol.internal_energy: " << endmol.internal_energy << "  endmol.contact_energy: " << endmol.contact_energy <<"  endmol.energy_tor_total:  " << endmol.energy_tor_total << "  endmol.hb_energy: " << endmol.hb_energy << endl;
        		        ...
        		        //best_models, float& best_Es
                        if (endmol.current_score < best_Es)
                        {
                            best_Es = endmol.current_score;
                            swapmol(best_models, endmol);
                            cout << "find model" << endl;
                        }
        ...
        //    method = 4;
        //}
        //if(maxsimulations == 40 )  method = 2;

        //if(maxsimulations<150 || maxsimulations>350)
        //if (maxsimulations%5 ==1)
        if ( !trans_method )
        {
            //if (maxsimulations <150)  method = 2;
           // if (maxsimulations%50 ==1)  method = 2;
                ...
                if (energy(amber, endmol, receptor, c_nrg, filevdw.c_str(), fileflex.c_str(), fileflex_drive_tbl.c_str(), grid_file, cutoff, biolip_matrix, sphx, sphy, sphz, bindsite_weight, protein_atom_index, ave, std, DensityMap, binding_sites, centers))
                {
                    ...
                    if (id == 1)
                    {
                        ...
                        //if (endmol.current_score < 1e+6)
                        {
                            //cout<<"endmol.internal_energy "<<endmol.internal_energy<<"original.internal_energy "<<original.internal_energy<<endl;
                            //cout<<"I am &&&&&&&&&&&&&&&&"<<endl;
                            cout << "rigid movement" << " original.current_score:" << original.current_score << "  original.CC: " << original.current_EM_score<< " original.intral_energy:" << original.intral_energy << " original.dis_energy:" << original.dis_energy << original.energy_tor_total << endl;
                            cout << "rigid movement" << " endmol.current_score:" << endmol.current_score << "  endmol.CC: " << endmol.current_EM_score << " endmol.intral_energy:" << endmol.intral_energy << " endmol.dis_energy:" << endmol.dis_energy << endmol.energy_tor_total <<endl;
                            vector <float> tmp(endmol.num_atoms, 0.0);
                            x.push_back(tmp);
                            y.push_back(tmp);
                            z.push_back(tmp);
...
bool generatationmol(DOCKMol & original,DOCKMol & endmol,int method,mt19937 &gen)
{
    ...
    if (method==1)
    {
        ...
        angle = c* rspacing;
        //cout<<"turn angle1 " <<angle<<endl;

    }
    if (method==2)
    {
        tspacing = rand0to1(gen);//1.0
        rspacing = largeangle(gen);
        angle=rspacing;
        //angle = 0.0;
	//cout<<"turn angle " <<angle<<endl;
        //cout<<"turn angle2 " <<angle<<endl;
    
    }
    if (method == 3)
    {
        tspacing = 0.5; // translate spacing
        rspacing = 180.0;
        c = randgeneration(gen);
        angle = c * rspacing;
        //cout<<"turn angle1 " <<angle<<endl;
    ...
    if (method == 4)
    {
        tspacing = 0.1; // translate spacing
        rspacing = 10;
        c = randgeneration(gen);
        angle = c * rspacing;
        //cout<<"turn angle1 " <<angle<<endl;
...
void swapmol(DOCKMol & mol1, DOCKMol & mol2)//put mol2 in mol1
{
    ...
    mol1.current_EM_score = mol2.current_EM_score;
}
