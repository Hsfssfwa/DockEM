...
int main(int argc, char ** argv){
    ...
    centers7[0] = centers7[0] / mol.num_atoms;
    centers7[1] = centers7[1] / mol.num_atoms;
    centers7[2] = centers7[2] / mol.num_atoms;
    ...
    theDensityMap.cart(cartbinding, fartbinding, 20);
    ...
    for (int i = 0; i < receptor_bindingsit.size(); i++)
    {
        ...
        {
            string VR1 = receptor_bindingsit[i].atom_name;//fin_maty[jj].elt_;
            string VR2 = pointsBx.atom_names[j];// fin_maty[t].elt_;
            float dis = 0.0;
            dis = dis + (receptor_bindingsit[i].x - pointsBx.x[j]) * (receptor_bindingsit[i].x - pointsBx.x[j]);
            dis = dis + (receptor_bindingsit[i].y - pointsBx.y[j]) * (receptor_bindingsit[i].y - pointsBx.y[j]);
            dis = dis + (receptor_bindingsit[i].z - pointsBx.z[j]) * (receptor_bindingsit[i].z - pointsBx.z[j]);
            dis_p = sqrt(dis);
    ...
    for (int i = 0; i < receptor_bindingsit.size(); i++)
    {
        for (int j = 0; j < pointsBxj.num_atoms; j++)
        {
            ...
            float dis = 0.0;
            dis = dis + (receptor_bindingsit[i].x - pointsBxj.x[j]) * (receptor_bindingsit[i].x - pointsBxj.x[j]);
            dis = dis + (receptor_bindingsit[i].y - pointsBxj.y[j]) * (receptor_bindingsit[i].y - pointsBxj.y[j]);
            dis = dis + (receptor_bindingsit[i].z - pointsBxj.z[j]) * (receptor_bindingsit[i].z - pointsBxj.z[j]);
            dis_p = sqrt(dis);
            //dis_p = Distance_point(fin_maty[jj].x_, fin_maty[t].x_);
            new_clash = new_clash + GetVdwEgCG(VR1, VR2, dis_p);
        }
...
//    pointsBx = best_model;
    ...
    //CC 0.05 0.001
    float supp_KT0s = 2;//;0.0001;1.5
    float supp_KT0e = 0.001;//;0.00001;0.001
    //1 0.01
    int supp_num1 = 20;
    int supp_num3 = 500;
    int supp_num4 = supp_num1;
    int supp_num2 = 15;//12 20
    // 300 50
    ...
    for (int kkk = 0; kkk < supp_num2; kkk++) // 2
    {
        ...
        for (int jjj = 0; jjj < supp_num1; jjj++)
        {
            ...
            for (int i = 0; i < receptor.num_atoms; i++)
            {
                for (int j = 0; j < point_mat.num_atoms; j++)
                {
                    string VR1 = receptor.atom_names[i];//fin_maty[jj].elt_;
                    string VR2 = point_mat.atom_names[j];// fin_maty[t].elt_;
                    float dis = 0.0;
                    dis = dis + (receptor.x[i] - point_mat.x[j]) * (receptor.x[i] - point_mat.x[j]);
                    dis = dis + (receptor.y[i] - point_mat.y[j]) * (receptor.y[i] - point_mat.y[j]);
                    dis = dis + (receptor.z[i] - point_mat.z[j]) * (receptor.z[i] - point_mat.z[j]);
                    dis_p = sqrt(dis);
                    //dis_p = Distance_point(fin_maty[jj].x_, fin_maty[t].x_);
                    new_clash = new_clash + GetVdwEgCG(VR1, VR2, dis_p);
                }
            }
            CC_i2 = theDensityMap.clCC_3(point_mat, receptor);
            new_CC2 = 1.0 - CC_i2;
            old_CC = new_CC2;
            old_clash = new_clash;
            old_dE = 500 * new_CC2 +old_clash;
            ...
            //    int tmp3 = 0;
                ...
                //    tmp3 = tmp3 + 1;
                //}
                generatationmol(fin_matx, fin_mat, 1, gen);
                CC_i2 = theDensityMap.clCC_3(fin_mat, receptor);
                new_clash = 0.0;
                dis_p = 0.0;
                for (int i = 0; i < receptor.num_atoms; i++)
                {
                    for (int j = 0; j < fin_mat.num_atoms; j++)
                    {
                        ...
                        float dis = 0.0;
                        dis = dis + (receptor.x[i] - fin_mat.x[j]) * (receptor.x[i] - fin_mat.x[j]);
                        dis = dis + (receptor.y[i] - fin_mat.y[j]) * (receptor.y[i] - fin_mat.y[j]);
                        dis = dis + (receptor.z[i] - fin_mat.z[j]) * (receptor.z[i] - fin_mat.z[j]);
                        dis_p = sqrt(dis);
                        //dis_p = Distance_point(fin_maty[jj].x_, fin_maty[t].x_);
                        new_clash = new_clash + GetVdwEgCG(VR1, VR2, dis_p);
                    }
...
/* for (int i = 0; i < fin_mat.num_atoms; i++)
                ...
                new_dE = 500 * (1.0 - CC_i2) +new_clash;
                new_CC = 1.0 - CC_i2;
    ...
    {
        cout << "eeeee" << best_Ex[i] << endl;
        string molfile1 = "test_" + std::to_string(i) + ".mol2";
        //ofstream clusterpdb(pdbfile.c_str());
        ofstream clustermol1(molfile1.c_str());
        string str1 = string(argv[1]);
        clustermol1 << "########## Name:" << endl;
        Write_Mol2(best_modelx[i], clustermol1, str1);
    ...
    {
        energy(amber, best_modelx[i], receptor, c_nrg, filevdw.c_str(), fileflex.c_str(), fileflex_drive_tbl.c_str(), grid_file, cutoff, biolip_matrix, sphx, sphy, sphz, bindsite_weight, protein_atom_index, ave, std, theDensityMap, receptor_bindingsit);
        ...
        matchvec.push_back(best_modelx[i]);
    }
  ...
  //REMC_flag = false; //not do remc
    ...
    vector<float> REMC_best_E(supp_num1, 1000000.0);
    ...
    if (REMC_flag == true)
    {
        ...
        if (REMC(amber, matchvec, receptor, c_nrg, filevdw.c_str(), fileflex.c_str(), fileflex_drive_tbl.c_str(), grid_file, 1, x, y, z, accenergy, Tmin, Tmax, outfile, gen, flexible_flag, biolip_matrix, swapnumber, MC_steps, sphx, sphy, sphz, bindsite_weight, protein_atom_index, ave, std, theDensityMap, REMC_best_model, REMC_best_E, receptor_bindingsit))
        {
